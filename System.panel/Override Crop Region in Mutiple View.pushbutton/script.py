...
from Autodesk.Revit.DB import  (FamilyInstance,FamilySymbol,Element,Transaction, OverrideGraphicSettings,
                            ViewSection,ViewPlan,ElementId ,FilteredElementCollector, BuiltInCategory, ElementCategoryFilter, LinePatternElement)
...
for view in selected_views:
    if type(view) is ViewPlan:  
        id_view = view.Id
        id_crop_str= str(id_view)
        id_crop_1 = int(id_crop_str)+1
        id_crop_2 = int(id_crop_str)+2
        view_change2 = view.SetElementOverrides(ElementId(id_crop_1),ogs)
        view_change3 = view.SetElementOverrides(ElementId(id_crop_2),ogs)  
    elif type(view) is ViewSection:
        cop_view_section = FilteredElementCollector(doc, view.Id).OfCategory(BuiltInCategory.OST_Viewers).WhereElementIsNotElementType().ToElementIds()
        view_change4 = view.SetElementOverrides(cop_view_section[0],ogs) 
